...
$archivo = "ratings.csv";
	...
	fputs($f,"Título;Director;Año;País;Puntuación;Fecha de votación\n");
...
$url="http://www.filmaffinity.com/es/userratings.php?user_id=".$iduser;
...
while ($i<$NP){
	...
	$url="http://www.filmaffinity.com/es/userratings.php?user_id=".$iduser."&p=".$i."&orderby=4";
	$html=file_get_html($url);
	
	$buscaFecha=stristr($html,"Votada el d");
	$buscaFecha2=stristr($buscaFecha,"</div>", true);
	$fechavot=substr($buscaFecha2,15);
	
// BUSCAR TÍTULO, AÑO, PAÍS Y DIRECTOR

while (true){
	
	$posfecha=strpos($html,"Votada el d");
...
$contenido=utf8_decode($titulo.";".$director.";".$anno.";".$pais.";".$rating.";".$fechavot."\n");
